...
try:
    from azureml.train.automl._remote_script import model_exp_wrapper
    does_sdk_have_best_run_model_exp_support = True
except Exception:
    does_sdk_have_best_run_model_exp_support = False
...
args = sys.argv
run_id = "11f9aa3d-8e11-4b59-9025-e3bb275a31b6"
aml_token = None
script_directory = None
print("Starting the Model explain run....")

preprocess = "True"  # PLACEHOLDER
run_id = "11f9aa3d-8e11-4b59-9025-e3bb275a31b6"  # PLACEHOLDER
automl_settings = {'is_subgraph_orchestration':False,'is_automode':True,'path':'./sample_projects/alldata','subscription_id':'ce9cac95-2300-44e5-a483-44c96c7f34f6','resource_group':'phaphontee-rg','workspace_name':'data-science-project','compute_target':'phaphontee3','iterations':1000,'primary_metric':'normalized_root_mean_squared_error','task_type':'regression','IsImageTask':False,'IsTextDNNTask':False,'test_size':0.2,'n_cross_validations':5,'preprocess':True,'is_timeseries':False,'time_column_name':None,'grain_column_names':None,'max_cores_per_iteration':-1,'max_concurrent_iterations':1,'max_nodes':1,'iteration_timeout_minutes':30,'enforce_time_on_windows':False,'experiment_timeout_minutes':360,'exit_score':'NaN','experiment_exit_score':'NaN','whitelist_models':None,'blacklist_models':None,'blacklist_algos':['TensorFlowDNN','TensorFlowLinearRegressor'],'auto_blacklist':False,'blacklist_samples_reached':False,'exclude_nan_labels':False,'verbosity':20,'model_explainability':True,'enable_onnx_compatible_models':False,'enable_feature_sweeping':False,'send_telemetry':True,'enable_early_stopping':False,'early_stopping_n_iters':20,'distributed_dnn_max_node_check':False,'enable_distributed_featurization':True,'enable_distributed_dnn_training':True,'enable_distributed_dnn_training_ort_ds':False,'ensemble_iterations':15,'enable_tf':False,'enable_cache':False,'enable_subsampling':False,'metric_operation':'minimize','enable_streaming':False,'use_incremental_learning_override':False,'force_streaming':False,'enable_dnn':False,'is_gpu_tmp':False,'enable_run_restructure':False,'featurization':'auto','vm_priority':'dedicated','label_column_name':'pm25','weight_column_name':None,'miro_flight':'default','many_models':False,'many_models_process_count_per_node':0,'automl_many_models_scenario':None,'enable_batch_run':True,'save_mlflow':True,'track_child_runs':True,'test_include_predictions_only':False,'enable_mltable_quick_profile':'True','has_multiple_series':False,'_enable_future_regressors':False,'enable_ensembling':True,'enable_stack_ensembling':False,'ensemble_download_models_timeout_sec':300.0,'stack_meta_learner_train_percentage':0.2} # PLACEHOLDER for AutoMLSettings
model_explain_run_type = "on_demand" # PLACEHOLDER for ModelExplainRunType
dataprep_json = None # PLACEHOLDER
mltable_data_json = "{\"Type\":\"MLTable\",\"TrainData\":{\"Uri\":\"azureml://locations/southeastasia/workspaces/8f4c2aef-7f73-4b55-9789-e5fd6eab7c7a/data/all/versions/1\",\"ResolvedUri\":\"azureml://locations/southeastasia/workspaces/8f4c2aef-7f73-4b55-9789-e5fd6eab7c7a/data/all/versions/1\",\"AssetId\":\"azureml://locations/southeastasia/workspaces/8f4c2aef-7f73-4b55-9789-e5fd6eab7c7a/data/all/versions/1\"},\"TestData\":null,\"ValidData\":null}" # PLACEHOLDER
on_demand_run_id = "alldata_regres_0"  # PLACEHOLDER
...
def model_explain_run():
    global script_directory
    result = None
    if model_explain_run_type == "on_demand":
        print("start on demand run model explain run.")
    elif model_explain_run_type == "best_run":
        print("start best run model explain run.")

    if does_sdk_have_best_run_model_exp_support:
        print("SDK has capability to process model explanations")
        result = model_exp_wrapper(
            script_directory=script_directory,
            automl_settings=automl_settings,
            run_id=run_id,
            child_run_id=on_demand_run_id,
            dataprep_json=dataprep_json,
            mltable_data_json=mltable_data_json,
            entry_point=entry_point
        )
        print("Model explain run completed successfully!")
    else:
        print("SDK does not have capability to process model explanations")

    return result

if __name__ == '__main__':
    result = model_explain_run()
    print(result)
